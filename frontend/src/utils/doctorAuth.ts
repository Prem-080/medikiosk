...
export type DoctorSession = { doctorId: string; name: string; specialty: string; hospitalName: string };
...
export function storeDoctorSession(doctor: DoctorSession) {
  ...
  document.cookie = 'medikiosk_doctor_session=active; path=/; max-age=86400; SameSite=Lax';
}

export function clearDoctorSession() {
  localStorage.removeItem(DOCTOR_KEY);
  document.cookie = 'medikiosk_doctor_session=; path=/; max-age=0; SameSite=Lax';
}
