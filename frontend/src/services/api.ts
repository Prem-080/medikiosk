...
export const api = {
  ...
  updatePatientIntake: async (sessionId: string, chiefComplaint: string) => {
    const res = await fetch(`${API_URL}/sessions/${sessionId}/patient-intake`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    if (!res.ok) {
      const err = await res.json();
      throw new Error(err.error || "Could not update consultation");
    }
    return res.json();
  },
  ...
  deletePatientSession: async (sessionId: string) => {
    ...
    if (!res.ok) {
      const err = await res.json();
      throw new Error(err.error || "Could not delete consultation");
    }
    return res.json();
  },
  ...
  doctorLogin: async (
    ...
    const res = await fetch(`${API_URL}/doctor-auth/login`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ doctorId, password, hospitalName }),
    });
    if (!res.ok) {
      const err = await res.json();
  ...
  validatePatientIntake: async (sessionId: string) => {
    const res = await fetch(
      `${API_URL}/doctor/case/${sessionId}/validate-intake`,
      { method: "POST" },
    );
    if (!res.ok) {
      const err = await res.json();
      throw new Error(err.error || "Could not validate intake");
    }
    return res.json();
  },
  ...
  finalizeConsultation: async (
    sessionId: string,
    data: { assessment: string; plan: string; followUpDate?: string },
  ) => {
    ...
    if (!res.ok) {
      const err = await res.json();
      throw new Error(err.error || "Could not finalise consultation");
    }
    return res.json();
  },
  ...
  sendCaseToAbha: async (sessionId: string) => {
    const res = await fetch(
      `${API_URL}/doctor/case/${sessionId}/send-to-abha`,
      { method: "POST" },
    );
    if (!res.ok) {
      const err = await res.json();
      throw new Error(err.error || "Could not prepare ABHA sharing");
    }
    return res.json();
  },
};
