  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="w-full max-w-lg bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
        <h2 className="text-3xl font-serif font-bold text-slate-800 mb-2">Consultation Details</h2>
        <p className="text-slate-500 mb-8">Select the hospital and doctor you are visiting today.</p>
        ...
        {mode === 'manual' ? (
          <div className="space-y-6">
            <div>
              <label className="flex items-center gap-2 text-slate-700 mb-2 font-medium">
                <Building2 size={18} className="text-primary" /> Hospital / Clinic
              </label>
              <select 
          ...
          <div className="flex flex-col items-center justify-center py-8">
            <div className="w-48 h-48 border-2 border-dashed border-primary/50 rounded-2xl flex flex-col items-center justify-center bg-primary/5 text-primary mb-6 cursor-pointer hover:bg-primary/10 transition-colors" onClick={handleQRMock}>
              <QrCode size={48} className="mb-2" />
              <span className="text-sm font-medium">Click to simulate scan</span>
            </div>
            <p className="text-center text-slate-500">Scan the Medikiosk QR code displayed at the reception desk.</p>
          </div>
        )}
      </div>
    </div>
