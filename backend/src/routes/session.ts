...
router.get('/patient/:patientId', async (req, res) => {
  try {
    const sessions = await ClinicalSession.find({ patientId: req.params.patientId }).sort({ createdAt: -1 });
    res.json({ sessions });
  } catch (err) {
...
router.patch('/:sessionId/patient-intake', async (req, res) => {
  try {
    ...
    if (session.status === 'reviewed') return res.status(400).json({ error: 'A practitioner-finalised consultation cannot be edited' });
    const { chiefComplaint } = req.body;
    if (!chiefComplaint?.trim()) return res.status(400).json({ error: 'Chief concern is required' });
    session.history = { ...session.history, chiefComplaint: chiefComplaint.trim() };
    await session.save();
    res.json({ session });
  } catch (err) {
...
router.delete('/:sessionId', async (req, res) => {
  try {
    const session = await ClinicalSession.findByIdAndDelete(req.params.sessionId);
    if (!session) return res.status(404).json({ error: 'Session not found' });
...
router.post('/start', async (req, res) => {
  try {
    ...
    const activeSession = await ClinicalSession.findOne({ patientId: pId, status: 'active' }).sort({ createdAt: -1 });
    if (activeSession) {
...
router.post('/:sessionId/chat', async (req, res) => {
  try {
    ...
    // Add patient message
    session.conversation.push({ role: 'patient', content: message, timestamp: new Date() });
    ...
    session.conversation.push({ role: 'ai', content: nextQuestion, timestamp: new Date() });
    ...
    if (nextQuestion.includes("collected enough information")) {
      session.history = MockAIService.generateStructuredHistory(patientResponses);
