...
router.get('/queue', async (req, res) => {
  try {
    ...
    const filters: Record<string, unknown> = { status: { $in: ['completed', 'reviewed'] } };
    ...
    if (doctorId) {
      ...
      filters.doctorId = doctorId === 'DR-1001'
...
router.get('/case/:sessionId', async (req, res) => {
  try {
    const session = await ClinicalSession.findById(req.params.sessionId).populate('patientId');
    if (!session) return res.status(404).json({ error: 'Session not found' });
    
    const documents = await MedicalDocument.find({ patientId: session.patientId });
...
router.post('/case/:sessionId/validate-intake', async (req, res) => {
  try {
    ...
    if (!session) return res.status(404).json({ error: 'Session not found' });
    session.practitionerReview = { ...(session.practitionerReview || {}), verified: false, intakeValidated: true, intakeValidatedAt: new Date() };
    await session.save();
    res.json({ session });
  } catch (err) {
...
router.post('/case/:sessionId/send-to-abha', async (req, res) => {
  try {
    const session = await ClinicalSession.findById(req.params.sessionId);
    if (!session) return res.status(404).json({ error: 'Session not found' });
    if (!session.practitionerReview?.verified) return res.status(400).json({ error: 'Finalise the practitioner review before ABHA sharing' });
    res.json({ status: 'demo_sent', message: 'Practitioner-approved record marked ready for ABHA sharing' });
  } catch (err) {
