...
router.post("/login", (req, res) => {
  ...
  if (
    doctorId?.trim().toUpperCase() !== DEMO_DOCTOR.doctorId ||
    password !== DEMO_DOCTOR.password ||
    hospitalName !== DEMO_DOCTOR.hospitalName
  ) {
    return res
      .status(401)
      .json({ error: "Invalid hospital, doctor ID, or password" });
  }
  res.json({
    doctor: {
      doctorId: DEMO_DOCTOR.doctorId,
